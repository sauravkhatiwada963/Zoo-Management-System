...
if(!isset($_SESSION)){session_start();
		...
		if( ($_SESSION['spons_archived'] =='Yes') || ($_SESSION['spons_signed'] =='No') ){
			session_unset(); 
			session_destroy();
					...
					<div class="sidebar-category sidebar-category-visible">
						<div class="category-content no-padding">
							<ul class="navigation navigation-main navigation-accordion">
								...
								<li >
									<a><i class="icon-paw"></i> <span>Animal Sponsorship</span></a>
									<ul class="active" style="display:block">
                                        <li  id="viewaviableanimalmenustab"><a><i class="icon-eye"></i>Animals </a></li>
                                        <li id="viewsponsoredanimalmenutab"><a><i class="icon-trophy4"></i>Sponsorsed Animals</a></li>
									</ul>
                                </li>

								<li id="viewbillingmenutab"><a><i class="icon-coins"></i>Billing</a></li>
								<!-- /main -->
							</ul>
						</div>
					</div>
...
<script>
 ...
 function loadavailableanimals(){
	$.ajax({
			url:"viewavailableanimalstemp.php",
			method:"POST",
			success:function(data)
			{
				$('.changingmain').html(data);
				$('#runningdashboard').html("Available Animals");	
			}
	});
 }

 $('#viewaviableanimalmenustab').click(function(){
	loadavailableanimals()
});

loadavailableanimals();

function loadsponsoredanimals(){
	$.ajax({
			url:"viewsponsoredanimalstemp.php",
			method:"POST",
			success:function(data)
			{
				$('.changingmain').html(data);
				$('#runningdashboard').html("Sponsored Animals");	
			}
	});
}
...
function loadbillings(){
	$.ajax({
			url:"mybillingtemp.php",
			method:"POST",
			success:function(data)
			{
				$('.changingmain').html(data);
				$('#runningdashboard').html("Billings");	
			}
	});
}

 $('#viewsponsoredanimalmenutab').click(function(){
	loadsponsoredanimals();
 });



$('#viewbillingmenutab').click(function(){
	loadbillings();
});
